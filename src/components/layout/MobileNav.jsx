...
const MobileNav = () => {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 inset-x-0 z-50 flex justify-around items-center px-4 py-2 bg-surface/95 backdrop-blur-lg border-t border-outline-variant shadow-[0_-4px_12px_rgba(0,0,0,0.05)] rounded-t-xl"
    ...
    >
      {items.map(({ to, icon, label }) => (
        <Link
          ...
          to={to}
          className={cn(
            'flex flex-col items-center justify-center transition-transform duration-200 active:scale-90 px-3 py-1',
            isActive(to)
        ...
        >
          ...
          <span className="text-[10px] mt-1 text-label-sm">{label}</span>
        </Link>
      ))}
    </nav>
  )
